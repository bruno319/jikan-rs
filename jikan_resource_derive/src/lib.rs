...
#[proc_macro_derive(Resource, attributes(uri_prefix))]
...
fn impl_resource_macro(ast: &syn::DeriveInput, prefix: Option<String>) -> TokenStream {
    ...
    let gen = match &ast.data {
        Data::Enum(ref data_enum) => {
            ...
            let snake_case_variant_names = data_enum.variants
                .iter()
                .map(|variant| to_snake_case(&mut variant.ident.to_string()));
            quote! {
                impl Resource for #enum_name {
                    fn uri(&self) -> String {
                        match self {
                            #(#repeated_enum_name::#variant_names => concat!(#prefix, #snake_case_variant_names).to_string(),)*
                        }
                    }
                }
            }
